...
- Executes `NAVIA_PATH.exe` synchronously in the background without opening console windows.
...
| `in_NavIAPathExe` | String | In (Optional) | Custom path to `NAVIA_PATH.exe` if placed outside standard directory. |
...
The activity automatically discovers `NAVIA_PATH.exe` using the following fallback order:
1. Argument `in_NavIAPathExe` if explicitly supplied.
2. Environment variable `NAVIA_PATH_EXE`.
3. Environment variable `NAVIA_PATH_HOME` (searches for `NAVIA_PATH_HOME\NAVIA_PATH.exe`).
4. Distribution parent directory (`..\..\NAVIA_PATH.exe`).
5. User local application directory `%LOCALAPPDATA%\NAVIA_PATH\NAVIA_PATH.exe`.
